...
export default function ProjectShowcaseSection() {
  return (
    <>
      <section className="relative flex flex-col justify-between items-center px-6 py-16 sm:px-12 lg:px-28 rounded-4xl gap-8 lg:py-28 w-full overflow-hidden">
        <div className="relative z-10 flex flex-col items-center w-full gap-8">
          <h2 className="self-stretch items-center text-center justify-center text-neutral-50 text-4xl font-bold font-['Signika_Negative'] leading-[48px]">Projects</h2>
          <div className="flex flex-col w-full gap-8">
            {projects.map((project, index) => (
              <ProjectListCard key={index} title={project.title} imageUrl={project.imageUrl} description={project.description} categories={project.categories} />
            ))}
          </div>
        </div>
